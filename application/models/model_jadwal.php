...
class Model_jadwal extends CI_Model
{
	...
	function generateJadwal()
	{
		...
		// Mengambil detail data dari kurikulum yang dipilih (kurikulum_detail)
		$kurikulumDetail = $this->db->get_where('kurikulum_detail', array('id_kurikulum' => $idkurikulum));

		// Ambil tahun akademik yang aktif
		$tahunakademik 	 = $this->db->get_where('tahun_akademik', array('is_aktif' => 'Y'))->row_array();


		foreach ($kurikulumDetail->result() as $row) {

			// ambil kelas berdasarkan tingkatan dan jurusan
			$kelasnya = $this->db->get_where('kelas', array('kd_jurusan' => $row->kd_jurusan, 'kd_tingkatan' => $row->kd_tingkatan));
			$id_guru = $this->db->get_where('guru', array('kd_mapel' => $row->kd_mapel))->row();
			...
			foreach ($kelasnya->result() as $row_kelas) {
				...
				$data = array(
					...
					'id_guru'			=> $id_guru ? $id_guru->id_guru : 0,
					'jam'				=> '',
					'kd_ruangan'		=> '000',
					'hari'				=> ''
				);
