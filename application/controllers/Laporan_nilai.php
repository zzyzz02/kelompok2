...
class Laporan_nilai extends CI_Controller
{
	...
	function index()
	{
		$walikelas 		= $this->db->get_where('walikelas', array('id_guru' => $this->session->userdata('id_guru')))->row_array();
	...
	function nilai_semester()
	{
		...
		$sqlSiswa = "SELECT ts.nama as nama_siswa, ts.nim, tj.nama_jurusan, tk.nama_kelas, tk.kd_tingkatan
	                    FROM riwayat_kelas as trk, siswa as ts, kelas as tk, jurusan as tj
	...
	{
		$sql   =  "SELECT sum(nilai)/count(nim) as nilai_rata_rata FROM nilai WHERE id_jadwal=$id_jadwal";
