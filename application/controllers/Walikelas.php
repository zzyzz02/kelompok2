...
class Walikelas extends CI_Controller
{
	...
	function data()
	{
		...
		// 	FROM walikelas AS tw, kelas AS tk, guru AS tg, jurusan AS tj, tingkatan_kelas AS ttk, 		tahun_akademik AS tta
		...
		$columns    = array(
			...
			array(
				...
				'formatter' => function ($d) {
					$walikelas = $this->db->get_where('walikelas', array('id_walikelas' => $d))->row_array();

					return cmb_dinamis('guru', 'guru', 'nama_guru', 'id_guru', $walikelas['id_guru'], "id='guru$d' onchange='updateWalikelas($d)'");
				}
			),
		);
	...
	function update_walikelas()
	{
		...
		$this->db->update('walikelas', array('id_guru' => $id_guru));
	}
}
